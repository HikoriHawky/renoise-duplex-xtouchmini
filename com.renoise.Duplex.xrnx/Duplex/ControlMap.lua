...
function ControlMap:_parse_xml(str,device_context)
  ...
  local function parseargs(str,empty_element,label)
    ...
    if (xarg.led_type) then
      xarg.led_type = tonumber(xarg.led_type)
    end
      ...
      if (label == "Group") then
        ...
        for idx,val in ipairs(toclose) do
          ...
          for idx2,_ in ipairs(toclose[idx]) do
            ...
            if not toclose[idx][idx2].xarg.led_mode then
              toclose[idx][idx2].xarg.led_mode  =  toclose[idx].xarg.led_mode
            end
