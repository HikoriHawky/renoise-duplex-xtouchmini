...
-- Use for _,parameter in ipairs(renoise.song().selected_track_device.parameters) do print(parameter.name) end to retrieve parameters
-- print(renoise.song().selected_track_device.name) for device name
...
Effect.available_mappings = {
  ...
  preset_name = {
    description = "Effect: Display preset name",
    component = UILabel,
  },
  param_names = {
...
function Effect:update()  
  ...
  -- update device label
  if self._device_name and rns.selected_track_device then
    self._device_name:set_text(rns.selected_track_device.display_name)
  else
  ...
  -- update preset label
  if self._preset_name and rns.selected_track_device and rns.selected_track_device.active_preset then
    self._preset_name:set_text(rns.selected_track_device.presets[rns.selected_track_device.active_preset])
  else
  ...
  if rns.selected_track_device then
    for _,custom_list in ipairs(custom) do
      if (rns.selected_track_device.name == custom_list[1]) then
...
function Effect:_set_fine(accel)
  TRACE("Effect:_set_fine()",accel)
...
function Effect:_update_ext_editor_button(device_idx)
  TRACE("Effect:_update_prev_next_device_buttons()",device_idx)
...
function Effect:_update_prev_next_bank_buttons()
  TRACE("Effect:_update_prev_next_bank_buttons()")
