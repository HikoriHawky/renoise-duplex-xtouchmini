...
-- Use for _,parameter in ipairs(renoise.song().selected_instrument.plugin_properties.plugin_device.parameters) do print(parameter.name) end to retrieve parameters
-- print(renoise.song().selected_instrument.plugin_properties.plugin_device.name) for device name
-- Choose random parameter that ideally stays off for a dummy param, use "-" for custom name

local custom =
{
  {
    "VST3: TAL-Togu Audio Line: TAL U-No-LX-V2",
    {
      "Global",
      {
        "keytranspose", "max voices", "Voice Mode", "Unisono Voices",
        "Unisono Detune", "Unisono Spread", "-", "FX Lock",
      },
      {
        "keytranspose", "max voices", "Voice Mode", "Unison Voices",
        "Unisono Detune", "Unisono Spread", "-", "FX Lock",
      },
    },
    {
      "Main",
      {
        "dco lfo value", "dco pwm value", "dco pwm mode", "dco pulse enabled",
        "dco saw enabled", "dco sub osc enabled", "dco sub osc volume", "dco noise volume",
      },
      {
        "dco lfo value", "dco pwm value", "dco pwm mode", "dco pulse enabled",
        "dco saw enabled", "dco sub osc enabled", "dco sub osc volume", "dco noise volume",
  ...
  {
    "VST3: discoDSP: Discovery Pro",
    { 
      "A",
      {
        "A:Lfo1_Sync", "A:Lfo1_Rate", "A:Lfo1_Sync", "A:Lfo1_Waveform",
        "A:Lfo1_Sync", "A:Lfo1_Destination", "A:Lfo1_Sync", "A:Lfo1_Amt",
      },
      {
        "LFO 1 SNC", "LFO 1 RATE", "-", "LFO 1 WAV",
        "-", "LFO 1 DST", "-", "LFO 1 AMOUNT",
      }
    },
    { 
      "B",
      {
        "B:Amp_Attack", "B:Amp_Decay",
      },
      {
        "Amp Attack", "Amp Decay",
      }
    },
    { 
      "C",
      {
        "C:Amp_Attack", "C:Amp_Decay",
      },
      {
        "Amp Attack", "Amp Decay",
      }
...
class 'InstrumentPlugin' (Automateable)

InstrumentPlugin.default_options = {
  include_parameters = {
    label = "Param. subset",
...
InstrumentPlugin.available_mappings = {

  parameters = {
    description = "InstrumentPlugin: Parameter value",
    component = UISlider,
    distributable = true,
    greedy = true,
    -- grid support 
  ...
  param_next = {
    description = "InstrumentPlugin: Next Parameter page",
    component = UIButton,
    distributable = true,
  },
  param_prev = {
    description = "InstrumentPlugin: Previous Parameter page",
    component = UIButton,
    distributable = true,
  },
  device_name = {
    description = "InstrumentPlugin: Display device name",
    component = UILabel,
  },
  param_names = {
    description = "InstrumentPlugin: Display parameter name",
    distributable = true,
    greedy = true,
  },
  param_values = {
    description = "InstrumentPlugin: Display parameter value",
    distrubutable = true,
    greedy = true,
  },
  param_active = {
...
InstrumentPlugin.default_palette = {
  -- parameter sliders
  ...
  -- parameter pages
  prev_param_on = {     color = {0xFF,0xFF,0xFF}, text = "Pg. ◄", val=true },
  prev_param_off = {    color = {0x00,0x00,0x00}, text = "Pg. ◄", val=false },
  next_param_on = {     color = {0xFF,0xFF,0xFF}, text = "Pg. ►", val=true },
  next_param_off = {    color = {0x00,0x00,0x00}, text = "Pg. ►", val=false },
  -- parameter active 
...
for k,v in pairs(Automateable.default_options) do
  InstrumentPlugin.default_options[k] = v
end
for k,v in pairs(Automateable.available_mappings) do
  InstrumentPlugin.available_mappings[k] = v
end
for k,v in pairs(Automateable.default_palette) do
  InstrumentPlugin.default_palette[k] = v
end
...
function InstrumentPlugin:__init(...)
  TRACE("InstrumentPlugin:__init", ...)
  ...
  --self:list_mappings_and_options(InstrumentPlugin)
...
function InstrumentPlugin:set_parameter(control_index, value, skip_event)
  TRACE("InstrumentPlugin:set_parameter", control_index, value, skip_event)
...
function InstrumentPlugin:update()  
  TRACE("InstrumentPlugin:update()")
  ...
  local device = rns.instruments[inst_idx].plugin_properties.plugin_device

  -- update device label
  if self._device_name and device then
  ...
  self:_update_ext_editor_button(inst_idx)
  
  self._custom_bank_count = 1
  local custom_banks = nil
  -- update custom parameter bank count
  if rns.selected_instrument.plugin_properties.plugin_device then
    for _,custom_list in ipairs(custom) do
      if (rns.selected_instrument.plugin_properties.plugin_device.name == custom_list[1]) then
...
function InstrumentPlugin:_build_app()
  TRACE("InstrumentPlugin:_build_app(")
  ...
  if not (self.mappings.parameters.group_name) then
    local msg = "InstrumentPlugin cannot initialize, the required mapping 'parameters' is missing"
...
function InstrumentPlugin:_get_selected_sample_device()
...
function InstrumentPlugin:_update_ext_editor_button(device_idx)
  TRACE("InstrumentPlugin:_update_prev_next_device_buttons()",device_idx)
...
function InstrumentPlugin:start_app()
  TRACE("InstrumentPlugin.start_app()")
...
function InstrumentPlugin:_get_parameter_by_index(idx)
  --TRACE("InstrumentPlugin._get_parameter_by_index()",idx)
...
function InstrumentPlugin:on_new_document()
  TRACE("InstrumentPlugin:on_new_document")
...
function InstrumentPlugin:_modify_ceiling(obj,prm)
...
function InstrumentPlugin:_get_quant_max(prm)
...
function InstrumentPlugin:_normalized_value_to_parameter_value(parameter, value)
...
function InstrumentPlugin:_parameter_value_to_normalized_value(parameter, value)
...
function InstrumentPlugin:on_idle()
  ...
  if self._track_update_requested then
    ...
    self._parameter_offset = 0
    self:_attach_to_track_devices(rns.selected_instrument)
    self._update_requested = true
  end
...
-- (updates InstrumentPlugin._num_devices)

function InstrumentPlugin:_define_parameters()
  TRACE("InstrumentPlugin:_define_parameters()")
...
function InstrumentPlugin:_get_num_devices()
  TRACE("InstrumentPlugin:_get_num_devices()")
...
function InstrumentPlugin:update_param_page()
  TRACE("InstrumentPlugin:update_param_page()")
...
function InstrumentPlugin:_set_fine(accel)
  TRACE("InstrumentPlugin:_set_fine()",accel)
...
function InstrumentPlugin:_update_prev_next_bank_buttons()
  TRACE("InstrumentPlugin:_update_prev_next_bank_buttons()")
...
function InstrumentPlugin:_attach_to_song()
  TRACE("InstrumentPlugin:_attach_to_song()")

  
  -- update on parameter changes in the song
  rns.selected_instrument_observable:add_notifier(
    function()
      TRACE("InstrumentPlugin:selected_sample_device_observable fired...")
  ...
  local new_song = true
  self:_attach_to_parameters(new_song)
  self:_attach_to_track_devices(rns.selected_instrument,new_song)
...
function InstrumentPlugin:_attach_to_track_devices(track,new_song)
  TRACE("InstrumentPlugin:_attach_to_track_devices",track,new_song)
  ...
  local inst_idx = rns.selected_instrument_index
  local device_observable = rns.instruments[inst_idx].plugin_properties.plugin_device_observable
  if (device_observable) then
    self._device_observables:insert(device_observable)
    device_observable:add_notifier(
      function()
        TRACE("InstrumentPlugin:devices_observable fired...")
...
function InstrumentPlugin:_attach_to_parameters(new_song)
  TRACE("InstrumentPlugin:_attach_to_parameters", new_song)
...
function InstrumentPlugin:_remove_notifiers(new_song,observables)
