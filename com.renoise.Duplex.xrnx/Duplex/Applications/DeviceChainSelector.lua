--[[============================================================================
-- Duplex.Application.DeviceChainSelector
============================================================================]]--
...
[View the README.md](https://github.com/renoise/xrnx/blob/master/Tools/com.renoise.Duplex.xrnx/Docs/Applications/DeviceChainSelector.md) (github)
...
class 'DeviceChainSelector' (Application)

DeviceChainSelector.default_options = {
  page_size = {
    label = "Page size",
...
DeviceChainSelector.available_mappings = {
  --[[
  prev_next_chain = {    
    description = "DeviceChainSelector: Select next/previous track",
    orientation = ORIENTATION.HORIZONTAL,    
  },
  ]]
  prev_chain = {    
    description = "DeviceChainSelector: Select previous device chain",
    component = UIButton,
  },
  next_chain = {    
    description = "DeviceChainSelector: Select next device chain",
    component = UIButton,
  },  
  prev_column = {    
    description = "DeviceChainSelector: Select previous column",
    component = UIButton,
  },
  next_column = {    
    description = "DeviceChainSelector: Select next column",
    component = UIButton,
  },
  --[[
  prev_next_page = {
    description = "DeviceChainSelector: Select track-page",
    orientation = ORIENTATION.HORIZONTAL,    
  },
  ]]
  prev_page = {
    description = "DeviceChainSelector: Select previous track-page",
    component = UIButton,
  },
  next_page = {
    description = "DeviceChainSelector: Select next track-page",
    component = UIButton,
  },
  select_track = {
    description = "DeviceChainSelector: Select active track",
    component = UISlider,
    orientation = ORIENTATION.HORIZONTAL,    
  },
  select_master = {
    description = "DeviceChainSelector: Select master-track",
    component = UIButton,
  },
  select_sends = {
    description = "DeviceChainSelector: Select 1st send-track",
    component = UIButton,
  },
  select_first = {
    description = "DeviceChainSelector: Select first track",
    component = UIButton,
  },
  chain_name = {
    description = "DeviceChainSelector: Display device chain",
    component = UILabel,
  }
}

DeviceChainSelector.default_palette = {
...
function DeviceChainSelector:__init(...)
  ...
  --self._prev_next_chain = nil
  self._prev_chain = nil
  self._next_chain = nil
  ...
  --self:list_mappings_and_options(DeviceChainSelector)
...
function DeviceChainSelector:start_app()
...
function DeviceChainSelector:on_new_document()
  TRACE("DeviceChainSelector:on_new_document()")
...
function DeviceChainSelector:on_idle()
...
function DeviceChainSelector:_attach_to_song(song)
  TRACE("DeviceChainSelector:_attach_to_song",song)
  ...
  song.selected_track_index_observable:add_notifier(
    function()
      TRACE("DeviceChainSelector:selected_track_observable fired...")
  ...
  song.tracks_observable:add_notifier(
    function()
      TRACE("DeviceChainSelector:tracks_changed fired...")
      -- request update on next idle() loop
...
function DeviceChainSelector:_get_track_page(track_idx)
...
function DeviceChainSelector:_get_page_width()
...
function DeviceChainSelector:update()
  TRACE("DeviceChainSelector:update()")
  ...
  --[[
  if (self._prev_next_chain) then
    local track_min = 1
    local track_max = #rns.tracks
    self._prev_next_chain:set_range(track_min,track_max)
    self._prev_next_chain:set_index(self._selected_track_index,skip_event)
  end
  ]]
  if (self._prev_chain) then
    if (self._selected_track_index <= 1) then
      self._prev_chain:set(self.palette.track_prev_off)
    else
      self._prev_chain:set(self.palette.track_prev_on)
    end
  end
  if (self._next_chain) then
    if (self._selected_track_index >= #rns.tracks) then
      self._next_chain:set(self.palette.track_next_off)
    else
      self._next_chain:set(self.palette.track_next_on)
    end
  end
  ...
  if self._chain_name then
    ...
    self._chain_name:set_text(rns.tracks[track_idx].name)
  end
...
function DeviceChainSelector:_build_app(song)
  TRACE("DeviceChainSelector:_build_app",song)
  ...
  --[[
  local map = self.mappings.prev_next_chain
  if map.group_name then
    TRACE("DeviceChainSelector:add next/previous track control (spinner)")
    ...
    self._prev_next_chain = c
  end
  ]]
  local map = self.mappings.prev_chain
  if map.group_name then
  ...
  local map = self.mappings.next_chain
  if map.group_name then
    local c = UIButton(self)
    c.group_name = map.group_name
    c.tooltip = map.description
  ...
  if map.group_name then
    TRACE("DeviceChainSelector:add previous/next page control (spinner)")
    local c = UISpinner(self)
    c.group_name = map.group_name
    c.tooltip = map.description
    c:set_pos(map.index or 1)
  ...
  if map.group_name then
    TRACE("DeviceChainSelector:add track-activator control (slider)")
  ...
  if map.group_name then
    TRACE("DeviceChainSelector:add first track select (pushbutton)")
    local c = UIButton(self)
    c.group_name = map.group_name
    c.tooltip = map.description
    c:set_pos(map.index)
  ...
  if map.group_name then
    TRACE("DeviceChainSelector:add master-track select (pushbutton)")
    local c = UIButton(self)
    c.group_name = map.group_name
    c.tooltip = map.description
    c:set_pos(map.index)
